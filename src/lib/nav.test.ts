...
describe("new-workspace cross-tab bus", () => {
  it("delivers the task pre-fill (name, taskId, claudePrompt) to a subscriber", () => {
    const received: NewWorkspaceRequest[] = [];
    const off = onNewWorkspace((r) => received.push(r));
    requestNewWorkspace({
      name: "Ship the delete button",
      taskId: "task-1",
      claudePrompt: "work on the task",
    });
    off();
    expect(received).toEqual([
      { name: "Ship the delete button", taskId: "task-1", claudePrompt: "work on the task" },
    ]);
  });
