 ...
 * core (over the control channel) to spawn `claude --remote-control` in a PTY
 * keyed `ws-claude:<workspaceId>`. That session lives in the core, independent
 * of the webview, so it can be started headlessly (e.g. from Telegram) and the
 * frontend attaches to the same live terminal later by id — and it's remotely
 * controllable from claude.ai/code or the Claude mobile app in the meantime.
 */
